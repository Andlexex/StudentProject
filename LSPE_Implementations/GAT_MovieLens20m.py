...
# Set the number of nodes (users and movies) ACHTUNG, DAS IST FALSCH!!!
num_nodes = len(filtered_rating_tensor)
...
for movie_id, genres in enumerate(genre_cols):
    for genre_id, genre_value in enumerate(genres):
        movie_genre_features[movie_id][genre_id] = genre_value
...
class GATModel(nn.Module):
    def __init__(self, hidden_channels):
        ...
        num_features = 24
        self.conv1 = GATv2Conv(num_features, 128, 1, edge_dim=1)
        self.conv2 = GATv2Conv(128, 1, 1)
        self.conv3 = GATv2Conv(64, 1, 1, edge_dim = 1)
        self.conv4 = GATv2Conv(32, 16, 1, edge_dim = 1)
        ...
        self.conv1_edge = GATv2Conv(1, hidden_channels, 1)
        self.conv2_edge = GATv2Conv(hidden_channels, 1, 1)
    ...
    def forward(self, x, edge_index, pos_embeddings, edge_attr):
       ...
       # x = self.conv3(x, edge_index)
        ...
        '''
        edge_attr = self.conv1_edge(edge_attr, edge_index)
        edge_attr = F.relu(edge_attr)
        edge_attr = self.conv2_edge(edge_attr, edge_index)
        '''
